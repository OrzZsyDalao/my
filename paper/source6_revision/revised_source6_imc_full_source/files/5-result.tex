...
respectively, and the observed coefficient difference is 0.631. Landlocked
units are not included in either group. The coefficients are descriptive
associations; the saved score-weighted per-unit table required for a country-
clustered uncertainty interval is not available.
...
additional coastal-mainland DNS units that are absent from the saved score-
weighted and Top-1 comparison. The monotonic sequence therefore describes the
available allocation-specific cohorts rather than a fully paired estimator
comparison. It shows that the geographic ordering does not arise under only
one allocation rule, but it does not isolate allocation as the cause of the
coefficient change. Without physical-route ground truth, the comparison also
does not establish that one rule recovers the true association.
...
interval for the difference includes zero, so the repeated direction across
allocation rules is reported as a descriptive robustness pattern rather than
as statistically established geographic separation.
...
measured snapshot. The existing results constrain possible explanations but
